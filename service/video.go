...
func (service *FeedService) GetFeed(userID uint64) (*response.FeedResponse, error) {
	...
	if service.Topic == "" {
		...
		resp, err1 := database.RPC.VideoRpc.SelectFeedVideoList(context.TODO(), &video.SelectFeedVideoListRequest{
			NumberVideos: constant.MaxVideoNumber,
			LastTime:     service.LatestTime,
		})
		videos = response.TransformVideoData(resp.VideoDatas)
		err = err1
	} else {
		//videos, err = database.SelectFeedVideoByTopic(constant.MaxVideoNumber, service.LatestTime, service.Topic)
		resp, err1 := database.RPC.VideoRpc.SelectFeedVideoByTopic(context.TODO(), &video.SelectFeedVideoByTopicRequest{
			NumberVideos: constant.MaxVideoNumber,
			LastTime:     service.LatestTime,
			Topic:        service.Topic,
		})
